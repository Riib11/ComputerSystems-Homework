...
class Cache::Impl {
    ...
    mutable evictor_obj_type * evictor_obj_;
...
public:
    ...
    void
    update_evicting(index_type size) {
        // RESIZING
        // cache full passed resize threshold, so need to resize
        if (USING_RESIZING_) {
            // resize until fits
            while (100.0 * memused_ / maxmem_ >= RESIZE_THRESHOLD_PERCENT_) {
                resize_cache();
            }    
        }
        // EVICTION
    ...
    get(key_type key, index_type& val_size) const {
        ...
        if (contains(key)) {
            ...
            auto pair = cache_.at(key);
            val_size = pair.second;
            return pair.first;
        }
        // key not in cache
        return NULL;
